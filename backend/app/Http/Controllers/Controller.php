...
abstract class Controller
{
    public static function saveReceipt($request, $fieldName = 'receipt')
    {
        ...
        if ($request->has($fieldName) && !empty($request->input($fieldName))) {
            return self::saveReceiptData($request->input($fieldName));
        }

        return null;
    }

    public static function saveReceiptData($data)
    {
        if (empty($data)) {
            return null;
        }
        ...
        if ($data instanceof \Illuminate\Http\UploadedFile) {
            return $data->store('payments', 'public');
        }

        if (is_string($data) && preg_match('/^data:image\/(\w+);base64,/', $data, $type)) {
            ...
            // Validate actual content MIME type
