...
import { Sparkles, LayoutGrid, Search, TrendingUp, TrendingDown, DollarSign, Tag, CheckCircle2, ChevronRight, ChevronLeft } from 'lucide-react';
import { apiClient, getStorageUrl } from '@/lib/api-client';

// Dresses Report Component with Full Pagination
export function DressesReport() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, viewMode, perPage]);

  useEffect(() => {
    let isMounted = true;
    Promise.all([
      apiClient.get('/dresses?per_page=1000'),
  ...
  // Compute stats per dress
  const dressStatsList = useMemo(() => {
    const bookingCounts = {};
    const revenueMap = {};

    bookings.forEach((b) => {
      [b.dress_id, b.dress_2_id, b.dress_3_id].filter(Boolean).forEach((dId) => {
        bookingCounts[dId] = (bookingCounts[dId] || 0) + 1;
        // Attribute proportional total amount or rental price
        const rent = parseFloat(b.total_amount || 0);
        revenueMap[dId] = (revenueMap[dId] || 0) + (rent > 0 ? rent : 0);
      });
    });

    return dresses.map((d) => {
      const bCount = bookingCounts[d.id] || d.bookings_count || 0;
      const rev = revenueMap[d.id] || (bCount * parseFloat(d.rental_price || 0));
      return {
        ...d,
        timesBooked: bCount,
        totalRevenue: rev,
  ...
  const processedList = useMemo(() => {
    ...
    if (viewMode === 'top') {
      return list.sort((a, b) => b.timesBooked - a.timesBooked || b.totalRevenue - a.totalRevenue);
  ...
  const statusCounts = useMemo(() => {
    const counts = { ready: 0, booked: 0, out: 0, cleaning: 0 };
    dresses.forEach((d) => {
  ...
  const paginatedList = useMemo(() => {
    return processedList.slice(startIndex, endIndex);
  }, [processedList, startIndex, endIndex]);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        <div className="bg-emerald-50/70 border border-emerald-200 rounded-2xl p-3 flex items-center justify-between">
          <div>
            ...
            <span className="font-mono text-base font-black text-emerald-950">{statusCounts.ready || 0} فستان</span>
          </div>
        ...
        <div className="bg-indigo-50/70 border border-indigo-200 rounded-2xl p-3 flex items-center justify-between">
          <div>
            ...
            <span className="font-mono text-base font-black text-indigo-950">{statusCounts.booked || 0} فستان</span>
          </div>
          <span className="text-xl">📅</span>
        </div>
        ...
        <div className="bg-blue-50/70 border border-blue-200 rounded-2xl p-3 flex items-center justify-between">
          <div>
            ...
            <span className="font-mono text-base font-black text-blue-950">{statusCounts.out || 0} فستان</span>
          </div>
          <span className="text-xl">👗</span>
        </div>
        ...
        <div className="bg-amber-50/70 border border-amber-200 rounded-2xl p-3 flex items-center justify-between">
          <div>
            ...
            <span className="font-mono text-base font-black text-amber-950">{statusCounts.cleaning || 0} فستان</span>
          </div>
          <span className="text-xl">🧼</span>
        </div>
      </div>

      {/* Dresses List Table & Mobile Cards */}
                  ...
                  <tr key={dress.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <span className={`px-2 py-0.5 rounded-lg text-[9.5px] font-black ${
                        dress.status === 'ready' ? 'bg-emerald-50 text-emerald-700' :
                        dress.status === 'out' ? 'bg-blue-50 text-blue-700' :
                        dress.status === 'cleaning' ? 'bg-amber-50 text-amber-700' :
                        'bg-slate-100 text-slate-600'
                      }`}>
                        {dress.status === 'ready' ? 'جاهز' :
                         dress.status === 'out' ? 'مستلم' :
                         dress.status === 'cleaning' ? 'دراي كلين' : dress.status}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 text-left font-mono font-black text-emerald-700">
                      {dress.totalRevenue.toLocaleString()} ج.م
                    </td>
                  </tr>
                ))
              )}
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2.5 border-b border-slate-200/60 pb-2.5">
                  ...
                  <span className={`px-2 py-0.5 rounded-lg text-[9.5px] font-black flex-shrink-0 ${
                    dress.status === 'ready' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                    dress.status === 'out' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
                    dress.status === 'cleaning' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
                    'bg-slate-100 text-slate-600'
                  }`}>
                    {dress.status === 'ready' ? 'جاهز' :
                     dress.status === 'out' ? 'مستلم' :
